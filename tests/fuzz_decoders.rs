...
//!
//! A VHD is four structures chained by offsets: the authoritative
//! footer at the END of the file, a mirror copy of it at the start, the
//! dynamic header behind that, and the block allocation table behind
//! that. Only the first is reachable from a byte slice; the rest need a
//! device, and which footer copy gets believed when the two disagree is
//! a decision a crafted image influences. So the corpus holds three
//! images `qemu-img` wrote and `image` mutates and opens them.
//!
//! One per shape that changes the read path: a dynamic disk, a fixed
//! one -- which is the raw data with a footer glued on, a different file
//! rather than a flag -- and a dynamic one written with `force_size`,
//! which fills the geometry fields in differently.
//!
...
fn targets() -> Vec<Target> {
    vec![
        ...
        Target {
            ...
            name: "footer",
            run: |b| {
                let _ = vhd::Footer::parse(b);
                let _ = vhd::footer::compute_checksum(b);
            },
        },
        Target {
            corpus: "dynamic_header",
            name: "dynamic_header",
            run: |b| {
                if let Ok(header) = vhd::dynamic::DynamicHeader::parse(b) {
                    let _ = header.bitmap_size_bytes();
                }
                let _ = vhd::dynamic::compute_checksum(b);
            },
        },
    ]
}
...
fn the_corpus_reads_back_what_qemu_img_wrote() {
    ...
    for (name, bytes) in images {
        ...
        let reader = vhd::VhdReader::open_on_device(dev)
            .unwrap_or_else(|e| panic!("{name}: an image qemu-img wrote would not open: {e}"));

        // The primary footer is intact in every committed image, so the
        // mirror should never have been needed. If this starts failing,
        // the footer at the end of the file stopped being believed --
        // which is a very different bug from a read returning the wrong
        // bytes, and would otherwise be invisible.
        assert!(
            !reader.footer_recovered_from_mirror(),
            "{name}: fell back to the mirror footer on an image whose primary is intact"
        );
        ...
        // that block, which is the property an unallocated block has and
        // the one an allocation-table error breaks.
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            ...
            let _ = writeln!(
                ...
                "\nhung: no progress for {:?} at {}\n\
                 A decoder did not return. A block allocation table whose entries point \
                 back into itself looks exactly like this.",
                DEADLINE,
                describe(&current),
            );
